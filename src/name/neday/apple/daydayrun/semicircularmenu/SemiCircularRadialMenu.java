...
package name.neday.apple.daydayrun.semicircularmenu;
...
//						.setBackgroundColor(mMenuItems.get(mPressedMenuItemID).getMenuSelectedColor());
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if(w > h) {
			mDiameter = h;
			mRadius = (float) (mDiameter/1.3 - (getPaddingTop() + getPaddingBottom()));
		} else {
			mDiameter = w;
			mRadius = (float) (mDiameter/1.3 - (getPaddingLeft() + getPaddingRight()));
		}
